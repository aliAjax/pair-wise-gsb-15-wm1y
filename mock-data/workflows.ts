import type {FlowEdge,FlowNode,PublishRecord,Version,Workflow} from '../src/types';
...
export const workflows:Workflow[]=names.map((name,i)=>{
 ...
 const versions:Version[]=[{version:1,createdAt:'2026-06-12 10:00',note:'初始化流程结构',nodes:oldNodes,edges:oldEdges}];
 if(version>=2) versions.push({version:2,createdAt:'2026-07-01 16:20',note:'增加金额分支与通知节点',nodes:graph.nodes,edges:graph.edges});
 if(version>=3) versions.push({version:3,createdAt:'2026-07-05 11:10',note:'优化审批链路与通知规则',nodes:graph.nodes,edges:graph.edges});
 const records:PublishRecord[]=status==='published'?[{version,publishedAt:'2026-07-08 14:30',note:'首次发布上线',frozen:true,dispositions:[]}]:[];
 return {id:'wf-'+(i+1),name,domain:['财务','采购','人力资源','IT服务','法务'][i%5],status,version,editor:['林秋','陈默','周礼','王宁'][i%4],updatedAt:`2026-07-${String(10-i%9).padStart(2,'0')} ${9+i%8}:20`,publishedAt:status==='published'?'2026-07-08 14:30':undefined,abnormalCount:i===7?0:i%4,nodes:graph.nodes,edges:graph.edges,versions,records};
});
