...
Playwright 会自动启动 Vite（端口 4173），串行验证 Dashboard、编辑器审批配置、双区域校验、条件修复、动态表单预览、发布联动、异常实例详情、版本比较和历史恢复，以及发布影响预检（差异比对、实例逐项处置门禁、执行版本绑定、撤回与带原因再发布、刷新后记录一致）。测试失败时会保留截图和 trace。
...
6. 返回并发布：先进入发布影响预检，比对节点新增/移除/配置变化，逐项处置受影响实例后确认发布；在 Dashboard 和 Workflows 检查状态及版本。
7. 进入运行监控，筛选异常并打开实例，查看时间线、高亮节点及实例实际执行版本（冻结快照）。
8. 从编辑器进入版本历史，选择两个版本比较，再恢复旧版本为草稿；或撤回当前发布回到上一快照，后续改动需填写变更原因才能再次发布。
